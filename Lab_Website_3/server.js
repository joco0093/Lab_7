...
db.any(query)
	...
	app.get('/player_info/select_player', function(req, res, player_id) {
		var query1 = 'SELECT name, id FROM football_players;';
		var query2 = 'SELECT * FROM football_players WHERE name = ';
		query2 += player_id;
		query2 += ';';
		var query3 = 'SELECT COUNT(*) FROM football_players WHERE  ';
		db.task('get-everything', task => {
	    return task.batch([
	        task.any(query1),
	        task.any(query2),
